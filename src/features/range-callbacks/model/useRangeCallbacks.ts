 ...
 *
 * - notifyChange: rAF throttle. 한 프레임에 여러 번 불려도 onRangeChange는 프레임 끝에 최신 값으로 1회만 호출된다.
 * - begin/endInteraction: "한 번의 조작" 세션. end 시점에 시작 대비 Range가 실제로 바뀐 경우에만 onRangeCommit을 1회 호출한다.
 * - end는 멱등 — pointerup과 lostpointercapture가 겹쳐 들어와도 두 번째는 무시된다.
 * - 사용자 콜백은 latest ref로 읽는다 — 인라인 함수를 넘겨도 배선이 다시 만들어지지 않고, rAF가 실행되는 시점의 최신 콜백이 호출된다.
 */
import { useCallback, useEffect, useRef } from "react";
import { isSameRange, type Range } from "../../../entities/range";
