 ...
 * Core 공통 타입.
 *
 * core는 순수 로직 계층이다 — Recharts/React를 비롯한 어떤 렌더링 지식도
 * 갖지 않는다 (ESLint no-restricted-imports로 강제).
 */
 ...
 *
 * 이 불변 조건의 보장은 모든 Range 변경 경로가 통과하는
 * 단일 clampRange의 책임이다.
 */
 ...
 * index만 결과로 돌려주면 recharts 계층이 `data[index]`로 원본 datum을 복원한다.
 */
